...
{
    aio_set_fd_handler(qemu_get_current_aio_context(), device->socket_write_handle, true, NULL, NULL, NULL, NULL);
}

static void unregister_read_handle(KPEDIState* device)
{
    aio_set_fd_handler(qemu_get_current_aio_context(), device->socket_read_handle, true, NULL, NULL, NULL, NULL);
}
